...
import {
  objectRenameFields,
  dottedFieldToNestedObject,
} from '../helpers/converTo';
...
import { compressionLogoFile } from '../helpers/image';
import { ContextUpload, PluginFile, UploadFiles } from '../plugins/upload';
...
export default class ApiController {
  ...
  async createNewObject({
    ...
    set,
    upload,
  }: ContextUpload<{ body: ObjectRequest }>): Promise<
    ResponceWithData<Objects> | ReponceWithReason
  > {
    const store: { upload: UploadFiles } = {
      upload: {},
    };
    ...
    if (
      store?.upload === undefined ||
      (body.photos === undefined && body.photosLayout === undefined)
    )
      return responce.failureWithReason({
        set,
        reason: 'У обьекта не может не быть фотки',
      });

    if (store.upload.photos === undefined)
    ...
    if (store.upload.photosLayout === undefined)
      return responce.failureWithError({
        set,
        error: {
          field: 'photoLayout',
    ...
    if (!!body.tentantLogo) {
      const compLogo = await compressionLogoFile(
        store.upload.tentantLogo as PluginFile,
      );

      newObject.tentantLogo = new Images(compLogo.filename);
    }
    ...
    const newImages = Array.isArray(store.upload.photos)
        ? store.upload.photos.map(
            uploadedPhoto => new Images(uploadedPhoto.filename),
          )
        : [new Images(store.upload.photos.filename)],
      newLayoutImages = Array.isArray(store.upload.photosLayout)
        ? store.upload.photosLayout.map(
            uploadLayoutPhoto => new Images(uploadLayoutPhoto.filename),
          )
  ...
  async addTentantToObject({
    body,
    set,
    params,
  }: ObjectAddNewTenantRequest): Promise<
    ResponceWithErrors | ResponceWithError<string> | ResponceWithData<Objects>
  > {
    const getObject: Objects | null = await orm.findOne(
      Objects,
      {
        id: params.id,
        type: 'sale-business',
    ...
    if (getObject === null)
      return responce.failureNotFound({
        set,
        error: {
          field: 'id',
    ...
    for (let tenant of body.tentants) {
      ...
      if (findTetant === null)
        errorAddTentant.push({
          field: tenant.tentantId,
          message: 'Не найден!',
        });
      else {
        const findIndexTentantInObject: number =
          getObject.tenantsInfo?.findIndex(
            objTentant => objTentant.tentantId === tenant.tentantId,
          );
    ...
    if (errorAddTentant.length > 0)
      return responce.failureWithErrors({ set, errors: errorAddTentant });
  ...
  async createNewTentant({
    body,
    set,
    upload,
  }: TenantCreateNewRequest): Promise<ResponceWithData<Tenant>> {
    const tryFindTentant = await orm.findOne(Tenant, {
      name: body.name,
    });

    if (tryFindTentant !== null)
      return responce.failureWithError({
        set,
        error: {
          field: 'name',
    ...
    const newTentant: Tenant = new Tenant({
      name: body.name,
      logo: new Images(upload.files.logo.filename),
  ...
  async getObjects({
    set,
    params,
  }: ObjectTypeRequest): Promise<
    ResponceWithData<
      Loaded<
        Objects,
        'images',
        'title' | 'images' | 'info' | 'address' | 'metro' | 'price',
        never
      >[]
    >
  > {
    const getObjects = await orm.findAll(Objects, {
      where: {
        type: !!params?.type ? params.type : { $ne: 'hidden' },
      },
      fields: [
        'images',
        'layoutImages',
  ...
  async getTentants({
    set,
  }: CustomRequestParams): Promise<
    ReponceWithoutData | ResponceWithData<Tenant[]>
  > {
  ...
  async getObjectBySlug({
    set,
    params,
  }: ObjectSlugRequest): Promise<
    ResponceWithError<string> | ResponceWithData<Objects>
  > {
    const getCurrentObject: Objects | null = await orm.findOne(
      Objects,
      {
        slug: params.slug,
      },
  ...
  async getArcivedObjects({ set }: ObjectTypeRequest) {
    const arcivedObjects = await orm.find(
      Objects,
      {
        deletedAt: {
          $ne: null,
  ...
  async editTentant({
    ...
    store,
    upload,
  }: Partial<TenantCreateNewRequest> & { params: { id: string } }) {
    delete body.logo;

    let editableTentant: Tenant | null = await orm.findOne(
      Tenant,
      {
        id: params.id,
      },
    ...
    if (!!upload.files.logo) {
      const newLogo: Images = new Images(upload.files.logo.filename);
  ...
  async editObject({
    ...
    request,
    upload,
  }: CustomRequestParams<Objects>) {
    try {
      const store = {};

      store.upload = upload.files;

      if (body?.isNew === true && body?.isNewPrice === true)
        return responce.failureWithError({
          set,
          error: {
            field: 'isNewPrice',
      ...
      let editableObject: Objects | null = await orm.findOne(
        Objects,
        {
          id: params.id,
        },
      ...
      if (editableObject === null)
        return responce.failureNotFound({
          set,
          error: { field: 'id', message: `Обьект ${params.id} не найден!` },
        });
      ...
      if (!!store.upload?.photos) {
        ...
        request.method === 'patch'
          ? editableObject.images.add(newImages)
          : editableObject.images.set(newImages);
      }
      if (!!store.upload.photosLayout) {
        ...
        request.method === 'patch'
          ? editableObject.layoutImages.add(newLayoutImages)
          : editableObject.layoutImages.set(newLayoutImages);
      }
      if (!!store.upload.tentantLogo) {
        ...
        const newTentantLogo = new Images(compLogo.filename);

        editableObject.tentantLogo = newTentantLogo;
      }

      if (body.tentantLogo === 'delete') {
        const tentantLogoRef = orm.getReference(
          Images,
          editableObject.tentantLogo?.id,
        );
      ...
      // toggle isNew fields
      if (editableObject.isNew && body?.isNewPrice === true)
        editableObject.isNew = false;
      if (editableObject.isNewPrice && body?.isNew === true)
        editableObject.isNewPrice = false;
  ...
  async editTentantInObject({ body, set, params }: ObjectEditTentantsRequest) {
    ...
    for (let tentantToEdit of body) {
      const findIndexInObject = getObject.tenantsInfo.findIndex(
        tentantInObject =>
          tentantInObject.tentantId === tentantToEdit.tentantId,
      );

      if (findIndexInObject === -1)
        errors.push({
          field: 'tenatantId',
  ...
  async reArchiveObject({
    ...
    'set' | 'params'
  >) {
    const restoreObjectOfDelete = await orm.findOne(
      Objects,
      {
        deletedAt: {
          $ne: null,
  ...
  async forceDeleteObject({
    ...
    'set' | 'params'
  >) {
    const getObjectToDelete = await orm.findOne(
      Objects,
      {
        id: params.id,
      },
  ...
  >): Promise<ReponceWithoutData | ResponceWithError<string>> {
    const getObjectToDelete: Objects | null = await orm.findOne(
      Objects,
      {
        id: params.id,
      },
  ...
  async deleteTentant({
    ...
    'set' | 'params'
  >): Promise<ReponceWithoutData | ResponceWithError<string>> {
    const getTentantToDelete: Tenant | null = await orm.findOne(
      Tenant,
      {
        id: params.id,
      },
    ...
    getTentantToDelete.objects.toArray().forEach(async objectRemoveTentant => {
      const getObjectRemoveTentant: Objects = await orm.findOne(
        Objects,
        {
      ...
      getObjectRemoveTentant.tenantsInfo?.splice(
        getObjectRemoveTentant.tenantsInfo.findIndex(
          tentantInObject =>
            tentantInObject.tentantId === getTentantToDelete.id,
        ),
        1,
      );
  ...
  async deleteTentantInObject({
    ...
    try {
      ...
      for (let tentantOfDelete of body) {
        const findIndexTentantInObject = getObject.tenantsInfo.findIndex(
          tenantInObject =>
            tenantInObject.tentantId === tentantOfDelete.tenatantId,
        );

        if (findIndexTentantInObject === -1)
          errors.push({
            field: 'tenatantId',
            message: 'Такого арендатора нет в обьекте',
          });
        else {
          const refGetTentantOfDelete: Tentant = getObject.tenants.find(
            tentant => tentant.id === tentantOfDelete.tenatantId,
          );
