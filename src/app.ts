...
import upload from './plugins/upload.ts';
  ...
  switch (code) {
    case 'NOT_FOUND':
      return responce.failureNotFound({
        set,
        error: { field: 'url', message: 'Не найдено!' },
      });
    case 'UNKNOWN':
      if (!!error?.error)
    ...
    case 'VALIDATION':
      if (error.all.findIndex(err => err.schema.anyOf?.length > 0) > -1)
        return responce.failureWithReason({
          set,
          statusCode: 400,
...
app.listen(port, () =>
  console.log(
    `Server run at: http://${app.server?.hostname}:${app.server?.port}${Bun.env
      .SERVER_BASE_PATH!}`,
  ),
);
